...
export const load = async ({fetch, params}) => {
  ...
  const post_res = await fetch(`/api/getPost?imageID=${id}`)
  ...
  const img_res = await fetch(`/api/getImage?imageID=${id}`)
  ...
  const name_res = await fetch(`/api/userName?userID=${post.post.author}`)
